...
from .fixmatch_utils import consistency_loss, Get_Scalar, CSLSupNonDiag, CSLUnSupNonDiag
...
class FixMatch:
    def __init__(self, net_builder, num_classes, ema_m, T, p_cutoff, lambda_u,\
                 ...
                 tb_log=None, logger=None,classes =[], kl_thresh=0.05):
        """
    ...
    def train(self, args, logger=None):
        ...
        for (x_lb, y_lb), (x_ulb_w, x_ulb_s, _) in zip(self.loader_dict['train_lb'], self.loader_dict['train_ulb']):
            if self.it % self.num_eval_iter == 0:
                criterionSup, criterionUnSup = self.validate(args)
            ...
            with amp_cm():
                ...
                sup_loss = criterionSup(logits_x_lb, y_lb, reduction='mean')
                unsup_loss, mask = criterionUnSup(logits_x_ulb_w, 
                                             logits_x_ulb_s)
                if args.vanilla_opt:
            ...
            if self.it % self.num_eval_iter == 0:
                ...
                metrics["mask"] = tb_dict['train/mask_ratio']
    ...
    def validate(self, args):
        ...
        CM = confusion_matrix(labels_, logits_, normalize="all")
        M, D = self.gain_matrix(CM)
        criterionSup, criterionUnsup = CSLSupNonDiag(M, D, device), CSLUnSupNonDiag(M, D, self.kl_thresh, device)
        return criterionSup, criterionUnsup
