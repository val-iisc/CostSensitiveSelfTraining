...
from train_utils import TBLog, get_SGD, get_cosine_schedule_with_warmup, param_group
from models.fixmatch.fixmatchMetricOpt import FixMatch

from ImbalancedSSL.data_utils import get_data_loader, split
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if args.vanilla_opt:
        if args.rank % ngpus_per_node == 0:
            print("using FM vanilla weight decay and cosine scheduler")
        ...
        if args.rank % ngpus_per_node == 0:
            print("using cudatom weight decay and step scheduler")
        param_groups = param_group(model.train_model,weight_decay=args.weight_decay)
        optimizer = torch.optim.SGD(param_groups, lr=args.lr, momentum=0.9, nesterov=True)
        scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer,
                                                            milestones=[int(0.5 * args.num_train_iter),
                                                                        int(0.75 * args.num_train_iter),
                                                                        int(0.9 * args.num_train_iter)], gamma = 0.1)
    ...
    if "cifar" in args.dataset:
        ...
        lb, ulb, val, eval = get_cifar("./data", args.dataset, args.N1, args.M1, args.val_size, args.rho_l, args.rho_u, False, 0, "exp")

    dset_dict = {'train_lb': lb, 'train_ulb': ulb, 'eval': eval, 'val': val}
    loader_dict = {}
    samp_per_class = list(Counter(lb.targets).values())
    prior = [x/sum(samp_per_class) for x in samp_per_class]
    model.prior = prior
...
if __name__ == "__main__":
    ...
    parser.add_argument('--vanilla_opt', type=bool, default=False)
    parser.add_argument('--ult', type=bool, default=False)
